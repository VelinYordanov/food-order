...
public class RestaurantsOrdersServiceImpl implements RestaurantsOrdersService {
	...
	public RestaurantsOrdersServiceImpl(FoodOrderData foodOrderData, Mapper mapper,
			SimpMessagingTemplate messagingTemplate, DateService dateService) {
	...
	public Collection<GraphData<LocalDate, Long>> getOrderMonthlyGraphData(String restaurantId, int year, int month) {
		...
		for (int i = 1; i <= daysForMonth; i++) {
			...
			if (result.stream().filter(graphData -> date.isEqual(graphData.getX())).findFirst().isEmpty()) {
				result.add(new GraphData<LocalDate, Long>(date, 0l));
			}
		}

		return result.stream().sorted(Comparator.comparing(GraphData::getX)).collect(Collectors.toList());
	}
