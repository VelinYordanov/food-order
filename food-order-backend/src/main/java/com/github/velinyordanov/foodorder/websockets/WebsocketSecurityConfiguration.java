...
public class WebsocketSecurityConfiguration extends AbstractSecurityWebSocketMessageBrokerConfigurer {
	...
	protected void configureInbound(MessageSecurityMetadataSourceRegistry messages) {
		messages.simpDestMatchers("/notifications/restaurants/{restaurantId}/orders").access(
				"hasAuthority('ROLE_RESTAURANT') and @websocketSecurityConfiguration.verifyId(principal, message)")
				...
				.simpTypeMatchers(SimpMessageType.DISCONNECT, SimpMessageType.UNSUBSCRIBE).permitAll().anyMessage()
				.authenticated();
	}
